...
NSString *fileContent(NSString *path) {
    return [NSString stringWithContentsOfFile:path encoding:NSUTF8StringEncoding error:nil];
}

int compileShader(NSString *path, GLenum shaderType) {
    int success;
    char infoLog[512];
    const char *src = fileContent(path).UTF8String;
    ...
    if (!success) {
        ...
        NSLog(@"编译shader失败: %s %@", infoLog, path);
        return -1;
    }
    return shader;
}
...
int main(int argc, const char *argv[]) {
    ...
    GLFWwindow *w = glfwCreateWindow(width, height, "opengl-lesson2", NULL, NULL);
    if (!w) {
        NSLog(@"创建window 失败");
        return -1;
    }
